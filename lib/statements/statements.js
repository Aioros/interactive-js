...
module.exports = [
...

  {
    ...
    execute: async function() {
      try {
        await this.Engine.process(this.block);
      } catch(err) {
        let handler = Object.assign({}, this.handler, {
          type: "FunctionExpression",
          params: [this.handler.param]
        });
        await this.Engine.callFunction(handler, [err]);
      } finally {
...

  {
    type: "ThrowStatement",
    execute: async function() {
      throw (await this.Engine.process(this.argument)).unwrap();
    }
  }
