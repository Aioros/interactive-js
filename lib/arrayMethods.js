  ...
  return Object.assign(Array.prototype, {
      ...
      while (k < len) {
        ...
        if (k in O) {
          ...
          if (isInteractiveFunction)
            testResult = await callbackfn.call(T, kValue, k, O);
          else
      ...
      if (thisArg === undefined) {
        while (++i !== len) {
          if (i in this) {
            var value;
            if (isInteractiveFunction)
              value = await func.call(null, t[i], i, t);
            else
              value = func(t[i], i, t);
            if (value) {
              res[c++] = t[i];
        ...
        while (++i !== len) {
          if (i in this) {
            var value;
            if (isInteractiveFunction)
              value = await func.call(thisArg, t[i], i, t);
            else
    ...
    find: async function(predicate, findIndex = false) {
      ...
      while (k < len) {
        ...
        var test;
        if (isInteractiveFunction)
          test = await predicate.call(thisArg, kValue, k, o);
        else
    ...
    reduce: async function(callback /*, initialValue*/) {
      ...
      while (k < len) {
        if (k in o) {
          if (isInteractiveFunction)
            value = await callback.call(null, value, o[k], k, o);
          else
    ...
    reduceRight: async function(callback /*, initialValue*/) {
      ...
      for (; k >= 0; k--) {
        if (k in t) {
          if (isInteractiveFunction)
            value = await callback.call(null, value, t[k], k, t);
          else
    ...
    some: async function(fun, thisArg) {
      ...
      for (var i = 0; i < len; i++) {
        if (i in t) {
          var test;
          if (isInteractiveFunction)
            test = await fun.call(thisArg, t[i], i, t);
          else
      ...
      if (!isInteractiveFunction) {
        ...
        async function merge(left, right) {
          ...
          while (left.length && right.length) {
            var test = await compareFunction.call(null, left[0], right[0]);
            if (test > 0) {
