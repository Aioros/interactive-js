            ...
            if (i < targets.length && targets[i] !== null) {
              ...
              if (targets[i].type == "RestElement") {
                transExp.right = {
                  ...
                  // I use await here because it's my .map, not the original Array.prototype.map
                  // (alternatively, I could use Array.prototype.map.call)
                  elements: await right.value.slice(v).map(el => ({value: el}))
                };
